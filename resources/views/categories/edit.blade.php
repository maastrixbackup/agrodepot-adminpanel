<x-app-layout>
    ...
    <div class="imageContainer mb-3">
        <h3>Edit Banner</h3>
        <form method="POST" action="{{ route('categories.update',['category' => $data->category_id]) }}" class="row" enctype="multipart/form-data">
            @csrf            
            @method('PUT')
                ...
                <div>
                    <label for="flag">Parent</label>
                        ...
                        @foreach ($categories as $flag => $catName)
                           <option value="{{ $catName->category_id }}" {{ optional($data)->catName == $catName ? 'selected' : '' }}> {{ $catName->category_name }}
                        </option>
                        @endforeach
