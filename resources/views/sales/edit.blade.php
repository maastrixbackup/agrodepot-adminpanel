<x-app-layout>
    ...
    <div class="imageContainer mb-3">
        <h3>Edit User</h3>
        <form method="POST" action="{{ route('sales.update', ['sale' => $data->adv_id]) }}" class="row" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="col-10 form-group">
                ...
                <div class="row">
                    <div class="col-6"><label for="input40" class="col-sm-4 col-form-label">Select a Category</label>
                        <select name="category_id" class="form-control form-select" id="category">
                            ...
                            <option value="{{ $category->category_id }}" @if ($category->category_id == $data->category_id) selected @endif>
                    ...
                    <div class="col-6"><label for="input40" class="col-sm-4 col-form-label">Select a
                        ...
                        <select name="sub_cat_id" id="sub_cat_id" class="form-control form-select">
                            ...
                            <option value="{{ $subcategory->category_id }}" @if ($subcategory->category_id == $data->sub_cat_id) selected @endif>{{ $subcategory->category_name }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="mt-2">
                    ...
                    <input type="text" name="adv_name" class="form-control" id="adv_name" value="{{ old('adv_name', $data->adv_name) }}">
                ...
                <div>
                    ...
                    <input type="file" class="form-control" name="adv_img[]" id="adv_img" onchange="previewImages()" accept="image/*" multiple>
                ...
                <div class="row" id="brandModelRow_0">
                    <div class="col-6"><label for="input40" class="col-sm-4 col-form-label">Select a Brand</label>
                        <select name="adv_brand_id" id="adv_brand_id" class="form-control form-select">
                            ...
                            <option value="{{ $brand->brand_id }}" @if ($brand->brand_id == $data->adv_brand_id) selected @endif>
                                {{ $brand->brand_name }}
                            </option>
                            @endforeach
                ...
                <div class="row">
                    ...
                    <div class="row" id="brandModelRow_{{ $key + 1 }}">
                        <div class="col-6">
                            ...
                            <input type="text" name="adv_brand_id[{{ $brand->parent->brand_id }}]" value={{ $brand->parent->brand_name }} class="form-control" placeholder="Enter Brand">

                        </div>
                        <div class="col-6">
                            <label for="input40" class="col-sm-4 col-form-label"></label>
                            <input type="text" name="adv_model_id[{{ $brand->brand_id }}]" value={{ $brand->brand_name }} class="form-control" placeholder="Enter Model">
                        </div>

                        <div class="col-2">
                            <button type="button" onclick="removeBrandModelRow({{ $key + 1 }})">Remove</button>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="row">
                    <div class="mt-2 col-3">
                        ...
                        <input type="text" name="price" class="form-control" id="price" value="{{ old('price', $data->price) }}">

                    </div>
                    <div class="mt-2 col-3">
                        <label for="price">Price</label>
                        <input type="text" name="b2bprice" class="form-control" id="price" value="{{ old('b2bprice', $data->b2bprice) }}">
                ...
                <div class="mt-2">
                    ...
                    <input type="text" name="quantity" class="form-control" id="quantity" value="{{ old('quantity', $data->quantity) }}">

                </div>
                <div class="mt-2">
                    <label for="payment_mode">Payment Methods</label><br>
                    <input type="radio" name="payment_mode" id="payment_mode_cod" value="Cash on delivery" @if ($data->payment_mode == 'Cash on delivery') checked @endif> Cash on delivery

                    <input type="radio" name="payment_mode" id="payment_mode_ud" value="Upon delivery" @if ($data->payment_mode == 'Upon delivery') checked @endif> Upon delivery

                    <input type="radio" name="payment_mode" id="payment_mode_wt" value="Wire Transfer" @if ($data->payment_mode == 'Wire Transfer') checked @endif> Wire Transfer

                    <input type="radio" name="payment_mode" id="payment_mode_bc" value="Banking Card" @if ($data->payment_mode == 'Banking Card') checked @endif> Banking Card

                    <input type="radio" name="payment_mode" id="payment_mode_others" value="Others" @if ($data->payment_mode == 'Others') checked @endif> Others


                </div>
                <div class="mt-2">
                    <label for="delivery">Method Of Delivery</label><br>
                    <input type="checkbox" name="personal_teaching" id="personal_teaching" value="1" @if ($data->personal_teaching == 1) checked @endif> Personal Teaching<br>

                    <input type="checkbox" name="courier" id="courier" value="1" @if ($data->courier == 1) checked @endif> Courier Delivery Cost
                    <input type="text" name="courier_cost" id="courier_cost" value="{{ $data->courier_cost }}">
                    RON<br>

                    <input type="checkbox" name="free_courier" id="free_courier" value="1" @if ($data->free_courier == 1) checked @endif> Free delivery by courier<br>

                    <input type="checkbox" name="romanian_mail" id="romanian_mail" value="1" @if ($data->romanian_mail == 1) checked @endif> Romanian Mail Delivery Cost
                    <input type="text" name="romanian_mail_cost" id="romanian_mail_cost" value="{{ $data->romanian_mail_cost }}"> RON<br>

                    <input type="checkbox" name="free_romanian_mail" id="free_romanian_mail" value="1" @if ($data->free_romanian_mail == 1) checked @endif> Free Shipping by Mail<br>

                </div>
                <div class='col-6'>
                    <label for="input40" class="col-sm-4 col-form-label">Time Required</label>
                    <select name="time_required" class="form-control" id="ManageSaleTimeRequired" required="required">
